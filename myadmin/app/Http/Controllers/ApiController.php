 ...
 */
    ...
    public function login(Request $request) {
        // check keys are exist
        $this->checkKeys(array_keys($request->all()), array("email", "password", "user_type", "device_id", "device_token", "device_type"));
        try {
            // check mobile number or password exist
            if (Auth::attempt(["email" => $request->email, "password" => $request->password]) || Auth::attempt(["mobile_number" => $request->email, "password" => $request->password])) {
                ...
                if (Auth::user()->user_role[0]->role == $request->user_type) {
                    ...
                    if (Auth::user()->user_role[0]->role == "driver") {
                        ...
                        if (Auth::user()->is_verified) {
                            ...
                            else if(Auth::user()->status == 'IN'){
                                $this->response = array(
                                    "status" => 503,
                                    "message" => ResponseMessages::getStatusCodeMessages(503),
                                );
                            }
                            else {
                                $this->response = array(
        ...
        $this->checkKeys(array_keys($request->all()), array("name", "email", "mobile_number", "password", "device_id", "device_type", "country", "state", "city"));
            ...
            if (!$validate->fails()) {
                ...
                if (!User::where("email", $request->email)->first()) {
                    ...
                    // SendMail::sendWelcomeMail("Welcome User - Grocery Store", $user, null, "emails.user_registration");

                    $this->response = array(
                        "status" => 200,
                        "message" => ResponseMessages::getStatusCodeMessages(200),
                        'refer_message' => "",
                        "data" => User::select("*", DB::raw("CONCAT('" . url("public/uploads/profiles") . "/', profile_picture) profile_picture"))->where("id", $user->id)->first(),
                    );
                } else {
                    $this->response = array(
                        "status" => 104,
        ...
        try {
            ...
            if (!User::where("mobile_number", $request->mobile_number)->first()) {
                ...
                $this->response = array(
                    "status" => 200,
                    "message" => ResponseMessages::getStatusCodeMessages(200),
                    "data" => User::select("*", DB::raw("(select ROUND(AVG(ratings.rating)) from ratings where parent_id = users.id) as rating"), DB::raw("CONCAT('" . url("public/uploads/profiles") . "/', profile_picture) profile_picture"))->where("id", $user->id)->first(),
                );
            } else {
                $this->response = array(
                    "status" => 105,
    ...
    public function categoryList(Request $request) {
        ...
        try {
            ...
            $quick_products = Product::select("products.*", DB::raw("COALESCE(cart.quantity,0) as cart_quantity"), DB::raw("(0) as selected_index"), DB::raw("(0) as SecondLoad"), DB::raw("(0) as checkPickerLoad"), DB::raw("IF ((select count(favourite_products.product_id) from favourite_products where favourite_products.product_id = products.id and favourite_products.user_id='$user_id' and favourite_products.status='AC') > 0,'Favourites_selected.png','Favourites.png') as favourite"), DB::raw("CONCAT('" . url("public/uploads/products") . "/', products.image) image"), DB::raw("(select product_variations.price from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as price"), DB::raw("(select product_variations.weight from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as weight"), DB::raw("(select product_units.name from product_variations join product_units on product_variations.unit_id = product_units.id where product_variations.product_id = products.id and product_variations.status='AC' and product_units.status='AC' limit 1) as unit"), DB::raw("(select product_variations.special_price from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as special_price"), DB::raw("(select round((product_variations.price - product_variations.special_price)*100 / product_variations.price) from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as discount"))->join('product_variations', 'product_variations.product_id', '=', 'products.id')->leftJoin("cart", function ($join) use ($user_id) {
                $join->on("cart.product_id", "=", "products.id");
                $join->where(["cart.user_id" => $user_id, "cart.status" => "AC"]);
            })->with(['get_product_variations' => function ($q) {
                $q->with(['product_units' => function ($q) {
            ...
            $offered_products = Product::select("products.*", DB::raw("COALESCE(cart.quantity,0) as cart_quantity"), DB::raw("(0) as selected_index"), DB::raw("(0) as SecondLoad"), DB::raw("(0) as checkPickerLoad"), DB::raw("IF ((select count(favourite_products.product_id) from favourite_products where favourite_products.product_id = products.id and favourite_products.user_id='$user_id' and favourite_products.status='AC') > 0,'Favourites_selected.png','Favourites.png') as favourite"), DB::raw("CONCAT('" . url("public/uploads/products") . "/', products.image) image"), DB::raw("(select product_variations.price from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as price"), DB::raw("(select product_variations.weight from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as weight"), DB::raw("(select product_units.name from product_variations join product_units on product_variations.unit_id = product_units.id where product_variations.product_id = products.id and product_variations.status='AC' and product_units.status='AC' limit 1) as unit"), DB::raw("(select product_variations.special_price from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as special_price"), DB::raw("(select round((product_variations.price - product_variations.special_price)*100 / product_variations.price) from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as discount"))->join('product_variations', 'product_variations.product_id', '=', 'products.id')->leftJoin("cart", function ($join) use ($user_id) {
                $join->on("cart.product_id", "=", "products.id");
                $join->where(["cart.user_id" => $user_id, "cart.status" => "AC"]);
            })->with(['get_product_variations' => function ($q) {
                $q->with(['product_units' => function ($q) {
            ...
            //$Category = Category::select("category.*", DB::raw("CONCAT('" . url("public/uploads/categories") . "/', category.image) image"))->where("category.status", "AC")->groupBy('category.id')->get();
            $usercartcount = 0;
            $favouritecount = 0;
    ...
    public function productList(Request $request) {
        ...
        try {
            ...
            $products = Product::select("products.*", DB::raw("COALESCE(cart.quantity,0) as cart_quantity"), DB::raw("(0) as selected_index"), DB::raw("(0) as SecondLoad"), DB::raw("(0) as checkPickerLoad"), DB::raw("IF ((select count(favourite_products.product_id) from favourite_products where favourite_products.product_id = products.id and favourite_products.user_id='$user_id' and favourite_products.status='AC') > 0,'Favourites_selected.png','Favourites.png') as favourite"), DB::raw("CONCAT('" . url("public/uploads/products") . "/', products.image) image"), DB::raw("(select product_variations.price from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as price"), DB::raw("(select product_variations.special_price from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as special_price"), DB::raw("(select round((product_variations.price - product_variations.special_price)*100 / product_variations.price) from product_variations where product_variations.product_id = products.id and product_variations.status='AC' limit 1) as discount"))->join('product_variations', 'product_variations.product_id', '=', 'products.id')->leftJoin("cart", function ($join) use ($user_id) {
                ...
                $query->select('id', 'name as brand_name');
            }])->where(['products.status' => 'AC', 'category_id' => $request->category_id, 'product_variations.status' => 'AC'])->groupBy('products.id')->get();
            // print_r(DB::getQueryLog());
            // die;
            if ($products->count() > 0) {
                $this->response = array(
    ...
    public function singleproductList(Request $request) {
        ...
        try {
            ...
            // ->first();
            $products = Product::select("*", DB::raw("CONCAT('" . url("public/uploads/products") . "/', image) image"))->with(['get_product_variations' => function ($q) {
    ...
    public function addToCart(Request $request) {
        ...
        try {
                ...
                $close_order =strtotime(date('G:i',strtotime('23:30')));
                ...
                if($now <= $close_order)
                {
                        ...
                        if ($addcart = Cart::where(['user_id' => $request->user_id, 'product_id' => $request->product_id, 'status' => 'AC'])->first()) {
                            ...
                            // $addcart->to_date = $request->to_date;
                            $addcart->save();
                        } else {
                            $addcart = new Cart();
                            $addcart->user_id = $request->user_id;
                ...
                else
                {
                    $this->response = array(
                        "status" => 505,
                        "message" => ResponseMessages::getStatusCodeMessages(505),
                    );
    ...
    public function updateCartFromOther(Request $request) {
        $this->checkKeys(array_keys($request->all()), array("product_id", "user_id", "cart_id", "update_type"));
        try {
            if ($addcart = Cart::where(['product_id' => $request->product_id, 'status' => 'AC'])->first()) {
                ...
                $addcart->save();
                $usercartsitem = Cart::select('cart.id', 'cart.product_id', 'cart.quantity', 'cart.scheduled', 'cart.from_date', 'cart.to_date', DB::raw("(cart.cgst+cart.sgst) as gst"), DB::raw("(products.cgst+products.sgst) as product_gst"), 'products.igst as product_igst', 'cart.total_without_tax', 'cart.total_with_tax', 'products.name as product_name', 'products.quantity as product_quantity', DB::raw("CONCAT('" . url("public/uploads/products") . "/', products.image) image"), 'product_variations.weight', 'product_variations.price as product_price', 'brands.name as brand_name', 'product_units.name as unit_name', 'product_units.code as unit_code')
                    ->join('products', 'cart.product_id', '=', 'products.id')
                    ->join('product_variations', 'cart.product_variation_id', '=', 'product_variations.id')
                    ->join('brands', 'brands.id', '=', 'products.brand_id')
                    ->join('product_units', 'product_units.id', '=', 'product_variations.unit_id')->where('cart.user_id', $request->user_id)->where('cart.status', 'AC')->get();
    ...
    public function updateCart(Request $request) {
        $this->checkKeys(array_keys($request->all()), array("product_id", "user_id", "update_type"));
        try {
    ...
    public function checkOut(Request $request) {
        ...
        try {
            ...
            // print_r(DB::getQueryLog());
                ...
                } elseif ($a = Cart::where('from_date', "<=", $curr)->where("to_date", ">=", $curr)->where(['user_id' => $request->user_id, 'status' => 'AC'])->first()) {
                    $a->from_date = date('Y-m-d');
                    $a->save();
                    OrderDelivery::where("cart_id", $a->id)->delete();
                }
                ...
                $usercartsitem = Cart::select('cart.id', 'cart.product_id', 'cart.quantity', 'cart.scheduled', 'cart.from_date', 'cart.to_date', DB::raw("(cart.cgst+cart.sgst) as gst"), DB::raw("(products.cgst+products.sgst) as product_gst"), 'products.igst as product_igst', 'cart.total_without_tax', 'cart.total_with_tax', 'products.name as product_name', 'products.quantity as product_quantity', DB::raw("CONCAT('" . url("public/uploads/products") . "/', products.image) image"), 'product_variations.weight', 'brands.name as brand_name', 'product_units.name as unit_name', 'product_variations.price as product_price', 'product_units.code as unit_code')
                    ->join('products', 'cart.product_id', '=', 'products.id')
                    ->join('product_variations', 'cart.product_variation_id', '=', 'product_variations.id')
                    ->join('brands', 'brands.id', '=', 'products.brand_id')
                    ->join('product_units', 'product_units.id', '=', 'product_variations.unit_id')->where('cart.user_id', $request->user_id)->where('cart.status', 'AC')->get();

                $user_aderss = UserAddress::select('id', 'address_type', 'default_address', DB::raw("CONCAT_WS(', ',house_no, apartment_name, street_details, landmark_details, area_details, city, state, pincode) as full_address"), 'mobile_number')->where(['user_id' => $request->user_id, 'status' => 'AC'])->where("default_address", 1)->where("status", "AC")->first();

                $usercartcount = Cart::where('cart.user_id', $request->user_id)->where('cart.status', 'AC')->count();
                $favouritecount = FavouriteProduct::where('favourite_products.user_id', $request->user_id)->where('favourite_products.status', 'AC')->count();

                if ($allcartitem->count() > 0) {
                    $cartTotal = Cart::select("id", DB::raw("SUM(cart.total_with_tax) as cart_total"))->where(['user_id' => $request->user_id, 'status' => 'AC'])->groupBy("user_id")->first();
                    $totalpay  = ($cartTotal->cart_total + $this->getBusRuleRef("delivery_charges"));
                    $this->response = array(
                        "status" => 200,
                        "message" => ResponseMessages::getStatusCodeMessages(200),
                        "data" => array('cart_data' => $usercartsitem, 'user_address' => $user_aderss),
                        "total_amount" => $cartTotal->cart_total,
                        "fav_count" => $favouritecount,
                        "cost" => $totalpay,
                        'delivery_charges' => $this->getBusRuleRef("delivery_charges"),
                        'minimum_order_amount' => $this->getBusRuleRef("minimum_order_amount"),
    ...
    public function CreateOrder(Request $request) {
        ...
        {
            $curr = date('Y-m-d');
            $check = DB::table('orders')->whereDate('created_at',$curr)
                        ->where('status','CM')
            ...
            else
            {
                ...
                if ($order->id) {
                    ...
                    if ($request->payment_method == "cod") {
                        ...
                        Cart::where(['user_id' => $request->user_id, 'status' => 'AC'])->update(["order_id" => $order->id, "status" => "PN"]);
                    }
                    $user = User::where('id',$request->user_id)->first();
                    if(date('D') === 'Sun'){
                        $message = MessageApi::send($user->mobile_number,
                            "Sobat Raja, mohon maaf hari Minggu Operational Raja Sayur libur, pesanan anda akan kami proses & dikirim hari selasa pagi. Thx"
                        );     
                    }
    ...
    public function getFavouriteProduct(Request $request) {
        ...
        try
        {
            $favouriteproducts = FavouriteProduct::select('products.id', 'products.name as product_name', DB::raw("CONCAT('" . url("public/uploads/products") . "/', products.image) image"), 'brands.name as brand_name', 'products.price')->join('products', 'favourite_products.product_id', '=', 'products.id')
    ...
    public function getCurrentdateOrders(Request $request) {
        ...
        try
        {
            ...
            $upcoming_orders = Order::select('orders.order_code', 'orders.id as order_id', 'user_addresses.name',
                ...
                ->join('brands', 'products.brand_id', '=', 'brands.id')
                ->join('user_addresses', 'orders.address_id', '=', 'user_addresses.id')
                ->where(['orders.driver_id' => $request->user_id, 'orders.status' => 'CM', 'cart.status' => 'CM'])->whereRaw("(select count(1) from order_delivery where order_delivery.cart_id=cart.id and order_delivery.status='AC' and order_delivery.order_date=CURDATE()) > 0")->groupBy('orders.id')->get();
            //    print_r(DB::getQueryLog());
    ...
    public function completedOrders(Request $request) {
        ...
        try
        {
            ...
            $upcoming_orders = Order::select('orders.order_code', 'orders.id as order_id', 'user_addresses.mobile_number',
                'user_addresses.name', 'orders.cost as order_total', DB::raw('group_concat(products.name) as names'), DB::raw("CONCAT_WS(', ',user_addresses.house_no, user_addresses.apartment_name, user_addresses.street_details, user_addresses.landmark_details, user_addresses.area_details, user_addresses.city, user_addresses.state, user_addresses.country, user_addresses.pincode) as full_address"))
                ->join('cart', 'cart.order_id', '=', 'orders.id')
                ->join('products', 'cart.product_id', '=', 'products.id')
                ->join('brands', 'products.brand_id', '=', 'brands.id')
                ->join('user_addresses', 'orders.address_id', '=', 'user_addresses.id')
                ->where(['orders.driver_id' => $request->user_id, 'orders.status' => 'CM', 'cart.status' => 'CM'])->whereRaw(DB::raw("(select count(1) from order_delivery where cart_id=cart.id and status IN ('CM','FL')) > 0"))->groupBy('orders.id')->get();
            if ($upcoming_orders->count() > 0) {
                $this->response = array(
                    "status" => 200,
                    "message" => ResponseMessages::getStatusCodeMessages(200),
    ...
    public function getPastOrders(Request $request) {
        ...
        try
        {
            ...
            $upcoming_orders = Order::select('orders.order_code', 'orders.cost as order_total', 'cart.id as cart_id', 'cart.from_date', 'cart.total_without_tax', 'cart.total_with_tax', 'cart.to_date', 'products.name as product_name', 'cart.product_id', 'cart.product_variation_id', 'cart.quantity', DB::raw("CONCAT('" . url("public/uploads/products") . "/', products.image) image"), 'brands.name as brand_name', DB::raw("CONCAT_WS(', ',user_addresses.house_no, user_addresses.apartment_name, user_addresses.street_details, user_addresses.landmark_details, user_addresses.area_details, user_addresses.city, user_addresses.state,user_addresses.country, user_addresses.pincode) as full_address"))
                ->join('cart', 'cart.order_id', '=', 'orders.id')
                ->join('products', 'cart.product_id', '=', 'products.id')
                ->join('brands', 'products.brand_id', '=', 'brands.id')
                ->join('user_addresses', 'orders.address_id', '=', 'user_addresses.id')
                ->where(['orders.user_id' => $request->user_id])->whereIn('orders.status', array('CM', 'CL'))->whereIn('cart.status', array('CM', 'CL'))->whereRaw(DB::raw("(select count(1) from order_delivery where cart_id=cart.id and status IN ('CM','FL','CL')) > 0"))->orderBy('orders.id', 'desc')->get();
            // print_r(DB::getQueryLog());
            if ($upcoming_orders->count() > 0) {
                $this->response = array(
                    "status" => 200,
    ...
    public function getUpcomingOrders(Request $request) {
        ...
        try
        {
            ...
            //        $q->select('id','name','brand_id', DB::raw("CONCAT('" . url("public/uploads/products") . "/', image) image"));
            ...
            $upcoming_orders = Order::select('orders.order_code', 'orders.cost as order_total', 'cart.id as cart_id', 'cart.from_date', 'cart.total_without_tax', 'cart.total_with_tax', 'cart.to_date', 'products.name as product_name', 'cart.product_id', 'cart.product_variation_id', 'cart.quantity', DB::raw("CONCAT('" . url("public/uploads/products") . "/', products.image) image"), 'brands.name as brand_name', DB::raw("CONCAT_WS(', ',user_addresses.house_no, user_addresses.apartment_name, user_addresses.street_details, user_addresses.landmark_details, user_addresses.area_details, user_addresses.city, user_addresses.state,user_addresses.country, user_addresses.pincode) as full_address"))
                ...
                ->join('brands', 'products.brand_id', '=', 'brands.id')
                ->join('user_addresses', 'orders.address_id', '=', 'user_addresses.id')
                ->where(['orders.user_id' => $request->user_id, 'orders.status' => 'CM', 'cart.status' => 'CM'])->whereRaw(DB::raw("(select count(1) from order_delivery where cart_id=cart.id and status='AC') > 0"))->orderBy('orders.id', 'desc')->get();
            //   print_r(DB::getQueryLog());
            //   die;
            
            $total = Order::select('cost')
    ...
    public function getOrderDetails(Request $request) {
        ...
        try
        {
            ...
            $orders = Cart::select('cart.id', "order_delivery.id as order_delivery_id", 'cart.product_id', 'cart.quantity', 'cart.scheduled', 'cart.from_date', 'cart.to_date', DB::raw("(cart.cgst+cart.sgst) as gst"), DB::raw("(products.cgst+products.sgst) as product_gst"), 'products.igst as product_igst', 'cart.total_without_tax', 'cart.total_with_tax', 'products.name as product_name', 'products.quantity as product_quantity', DB::raw("CONCAT('" . url("public/uploads/products") . "/', products.image) image"), 'product_variations.weight', 'brands.name as brand_name', 'product_units.name as unit_name', 'product_units.code as unit_code')
                ->join('products', 'cart.product_id', '=', 'products.id')
                ->join('product_variations', 'cart.product_variation_id', '=', 'product_variations.id')
                ->join('brands', 'brands.id', '=', 'products.brand_id')
            ...
            if ($request->type == "completed") {
                $orders = $orders->addSelect(DB::raw("'false' as visible"))->whereRaw(DB::raw("(select count(1) from order_delivery where cart_id=cart.id and status IN ('CM','FL')) > 0"))->groupBy("cart.product_id")->get();
                ...
                $orders = $orders->whereDate('order_delivery.order_date', '=', DB::raw('CURDATE()'))->where("order_delivery.status", "AC")->get();
            }
            //  print_r(DB::getQueryLog());
    ...
    public function deliveryStatus(Request $request) {
        ...
        try
        {
            if ($order = OrderDelivery::where(['id' => $request->order_delivery_id, 'status' => 'AC'])->first()) {
                $cartdata = Cart::where("id", $order->cart_id)->first();
                $orderdata = Order::where("id", $cartdata->order_id)->first();
                $user = User::where("id", $cartdata->user_id)->first();
                if ($request->status == "CM") {
                    $order->status = $request->status;
                    $notification = new Notification();
                    $notification->user_id = $cartdata->user_id;
                    $notification->order_id = $cartdata->order_id;
    ...
    public function cityList(Request $request) {
        ...
        try {
            // get city list based on state
            $StateCities = City::select("id", "name")->where(["state_id" => $request->state_id, 'status' => 'AC'])->get();
