...
<form id="paynow_form" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
     ...
     <input type="hidden" name="item_name" value="Grocery Item" />

     <input type="hidden" name="custom" value="{{$data->transaction_code}}" />
     <input type="hidden" name="item_name" value="Grocery Item">
     <INPUT TYPE="hidden" NAME="return" value="http://<?=$_SERVER['HTTP_HOST']?>/api/payment_success">
     <INPUT TYPE="hidden" NAME="cancel" value="http://<?=$_SERVER['HTTP_HOST']?>/api/payment_failed">
