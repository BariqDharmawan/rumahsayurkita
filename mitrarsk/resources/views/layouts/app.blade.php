...
<head>
  ...
  <title>Rumah Sayur Kita | Admin</title>
...
<script src="{{URL::asset('/public/plugins/bootstrap-material-datetimepicker/js/bootstrap-material-datetimepicker.js')}}"></script>
    ...
    <script type="text/javascript">
        $('div.alert').delay(10000).slideUp(500);
            ...
            let order = $("input[name='orderByDate']").val();
            if(order === ''){
                alert('ORDER BY DATE IS EMPTY!');
            }
            else{
                $("#allOrder").hide();
                $("#orderDate").show();
                $("#orderDate").html("<img style='max-width:30px' src='{{asset('public/images/25.gif')}}'> Wait...");
                $("#orderDate").load("{{url('/admin/order-by-date?date=')}}"+order);
            }
         });
         $.ajax({
             url:"{{ url('admin/getPendingUser')}}",
             type:'get',
             success:function(data){
                 if(data === 'false'){
                     $("#pendingUser").hide();
