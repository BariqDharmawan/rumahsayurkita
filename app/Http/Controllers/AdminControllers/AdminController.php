...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
		...
		$result['todayNetProfit'] = $this->getTodayNetProfit();
	...
	private function getTodayNetProfit()
	{
		$purchasedPrice = 0;
		$soldPrice = 0;
		...
		$orders = DB::table('orders')
			->where('customers_id', '!=', '')
			->where('date_purchased', '>=', $todayDate->format('Y-m-d H:m:s'))
			->get();
		...
		$todayRevenue = $soldPrice - $purchasedPrice;
		return [
			'date' => $todayDate->format('d-m-Y'),
			'value' => $todayRevenue
		];
	}
}
